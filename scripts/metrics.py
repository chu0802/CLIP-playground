import argparse
from scripts.utils import DEFAULT_OUTPUT_ROOT, DEFAULT_DATASET_SEQ
import json
import pandas as pd
...
def max_zero_shot_degradation(res_list):
    metric = {
        res.index[-1]: 100 * (res.iloc[:, -1].min() - DEFAULT_ZERO_SHOT_PERFORMANCE[res.index[-1]])
        for res in res_list
    }
    return metric_to_dataframe(metric, "zero-shot degradation")

def avg_final_performance(res_list):
    metric = 100 * pd.concat([res.iloc[-1][DEFAULT_DATASET_SEQ] for res in res_list], axis=1).mean(axis=1)
    return metric.to_frame("avg. final performance").T.round(2)

def parse_results(method_dir="split_teacher_config"):
    res_list = []
    for order in range(8):
        res_path = DEFAULT_OUTPUT_ROOT / f"order_{order}" / method_dir / "final_results.json"
